...
export default function ControlsPanel({
  ...
  const {
    ...
    applyPreset,
    PRESET_WIDTHS,
  } = useResizePresets(nat, limitToOrig, onPickSize);
    ...
    <div className="rounded-2xl border border-white/10 bg-white/[0.04] backdrop-blur-xl p-4 shadow-[0_8px_30px_rgba(0,0,0,0.25)]">
      ...
      <div className="mb-2">
        <div className="flex items-center justify-between mb-2">
          <span className="text-[12px] tracking-wide text-slate-400">
            Wybierz proporcje
          </span>
          <div className="inline-flex p-1 rounded-xl bg-white/5 ring-1 ring-white/10">
            <button
              ...
              className={`px-3 py-1.5 text-xs rounded-lg transition
                ${
                  !portrait
                    ? "bg-indigo-600 text-white shadow ring-1 ring-indigo-400/60"
          ...
          >
            <div className="flex items-center justify-between mt-3 mb-2">
              <span className="text-[12px] text-slate-400">
                Rozdzielczości {activeRatio.label} (
                {portrait ? "pion" : "poziom"})
              </span>
              <button
                type="button"
                onClick={() => pickRatio(activeRatio.id)}
            ...
            >
              {PRESET_WIDTHS[activeRatio.id].map((WL) => {
                ...
                const tooBig =
                  !!(limitToOrig && nat && (W > nat.w || H > nat.h));

                const isActive =
                  selectedPreset?.w === W && selectedPreset?.h === H;
                ...
                return (
                  <motion.button
                    ...
                    disabled={tooBig}
                    onClick={() =>
                      !tooBig && applyPreset(activeRatio.id, WL)
                    }
